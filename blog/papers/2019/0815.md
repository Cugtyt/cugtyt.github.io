...
坏处是，在原始的DARTS中，每个输出节点 `$\mathbf{x}_{j}$` 都需要从 {% raw %} $\{ \mathbf{x}_{0}, \mathbf{x}_{1}, \dots, \mathbf{x}_{j-1} \}$ {% endraw %} 挑选两个输入节点，权重分别为：{% raw %} $max_o \alpha ^ o _ {0, j}, max_o \alpha ^ o _ {1, j}, \dots, max_o \alpha ^ o _ {j-1, j}$ {% endraw %} 。但是这些结构参数是通过随机采样的通道来优化的，可能并不稳定，导致网络结构的波动变化。我们引入了边标准化，对每条边$(i, j)$显式赋予权重，记作 `$\beta_{i,j}$` ，这样，`$\mathbf{x}_{j}$` 的计算变成：
