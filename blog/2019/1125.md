...
- [数据角度](#数据角度)
  - [EfficientNet对于不同数据增强的实验结果](#efficientnet对于不同数据增强的实验结果)
- [模型角度](#模型角度)
- [Tricks](#tricks)
...
细粒度图像分类是在区分出基本类别的基础上，进行更精细的子类划分，如区分鸟的种类、车的款式、狗的品种等。细粒度图像相较于粗粒度图像具有更加相似的外观和特征，加之采集中存在姿态、视角、光照、遮挡、背景干扰等影响，导致数据呈现类间差异性大、类内差异性小的现象，从而使分类更加具有难度。
